...
class WhatsappSessionWebJSCore extends session_abc_1.WhatsappSession {
    ...
    async sendImage(request) {
        const chatId = this.ensureSuffix(request.chatId);
        const buffer = await (0, media_utils_1.resolveFileToBuffer)(request.file);
        const media = new whatsapp_web_js_1.MessageMedia(request.file.mimetype || 'image/jpeg', buffer.toString('base64'), request.file.filename || undefined);
        const options = Object.assign(Object.assign({}, this.getMessageOptions(request)), { caption: request.caption || undefined });
        return this.whatsapp.sendMessage(chatId, media, options);
    }
    async sendFile(request) {
        const chatId = this.ensureSuffix(request.chatId);
        const buffer = await (0, media_utils_1.resolveFileToBuffer)(request.file);
        const media = new whatsapp_web_js_1.MessageMedia(request.file.mimetype || 'application/octet-stream', buffer.toString('base64'), request.file.filename || 'file');
        const options = Object.assign(Object.assign({}, this.getMessageOptions(request)), { caption: request.caption || undefined, sendMediaAsDocument: true });
        return this.whatsapp.sendMessage(chatId, media, options);
    }
    async sendVoice(request) {
        const chatId = this.ensureSuffix(request.chatId);
        const buffer = await (0, media_utils_1.resolveFileToBuffer)(request.file);
        const media = new whatsapp_web_js_1.MessageMedia(request.file.mimetype || 'audio/ogg; codecs=opus', buffer.toString('base64'), 'voice-message.ogg');
        const options = Object.assign(Object.assign({}, this.getMessageOptions(request)), { sendAudioAsVoice: true });
        return this.whatsapp.sendMessage(chatId, media, options);
    }
